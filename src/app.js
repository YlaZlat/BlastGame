...
var GameLayer = cc.Layer.extend({
    ...
    //центральная кнопка   
        ...
        //правая кнопка
         var bonusesItemRight = new cc.MenuItemImage(res.bonusField_png, res.bonusField_png, this.buyMix2.bind(this));
        ...
        for (var i = 0; i < 18; i++) {
            for (var j = 0; j < 9; j++) {
                ...
                cell.sprite.textureFile = randomColor.texture;
                if(i ==9){indent=50};
    ...
    buyMix: function(){ // купить перемешивание поля
        ...
        this.addCoins(-this.mixCost);
         for (var i = 0; i < this.currentCubes.length; i++) {
                var randomColor = this.cubeColors[Math.floor(Math.random()*this.cubeColors.length)];
                cc.log (randomColor.texture);
                this.currentCubes[i].sprite.initWithFile(randomColor.texture);
                this.currentCubes[i].sprite.type = randomColor.color;
        }
    },
// перемешивание видимого поля
    buyMix2: function(){ // купить перемешивание поля
        if(coins < this.mixCost )return;
        this.addCoins(-this.mixCost);
        var currentColors = [];
            for (var i = 81; i < this.currentCubes.length; i++) {
                var currentColor = {color:this.currentCubes[i].sprite.type, texture: this.currentCubes[i].sprite.textureFile};
                currentColors.push(currentColor);
        }
         currentColors.sort(function() {return Math.random() - 0.5});

           for (var i = 81; i <  this.currentCubes.length; i++) {
            var randomColor = {color:currentColors[i-81].color, texture: currentColors[i-81].texture};
            this.currentCubes[i].sprite.initWithFile(randomColor.texture);
            this.currentCubes[i].sprite.textureFile = randomColor.texture;
            this.currentCubes[i].sprite.type = randomColor.color;
             cc.log(randomColor.texture);
             cc.log(randomColor.color);
           }
    },
                ...
                if(arr[i] && cc.rectContainsPoint(arr[i].sprite.getBoundingBox( ), this.field.currentTouch)) {
                    ...
                    for(var i = 0; i< arr.length; i++){
                        if(arr[i].sprite ==null) {
                            ...
                            arr[i].sprite.textureFile = randomColor.texture;
