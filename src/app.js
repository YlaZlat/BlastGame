                ...
                if(arr[i] && cc.rectContainsPoint(arr[i].sprite.getBoundingBox( ), this.field.currentTouch)) {
                    var explosionGroup = [arr[i],];
                        function seachExplosionGroup(arr, i){
                            ...
                            this.levelScore += (explosionGroup.length/2) * explosionGroup.length;
                            this.totalScore += (explosionGroup.length/2)*explosionGroup.length;
                            this.createPlusScore((explosionGroup.length/2) + "x" + explosionGroup.length, explosionGroup[0].position);
                            this.sliderUpdate();
